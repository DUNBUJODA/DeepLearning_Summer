...
def data(dir='v3_data_cur/'):
    ...
    return train_set_x.T, train_set_y.reshape(-1, train_set_y.shape[0]), \
           valid_set_x.T, valid_set_y.reshape(-1, valid_set_y.shape[0]), \
           test_set_x.T, test_set_y.reshape(-1, test_set_y.shape[0])
...
class MLP(object):
    ...
    def __init__(self, input_size: int, hidden_layers: list, output_size: int):
        ...
        cf.read(config_path + "config.ini")
        ...
        self.patience_increase = 4  # wait this much longer when a new best is
        # found
            ...
            for i in range(batch_num):
                ...
                if self.early_stop:
                    ...
                    if (iter + 1) % self.validation_frequency == 0:
                        ...
                        print(
                            'epoch %i, minibatch %i/%i, patience %f, train loss %f, validation error %f %%' %
                            (
                                l,
                                i,
                                batch_num,

...
def load_basketball_data(dir='v4_data_grey/'):
    ...
    train_y_cato = np.zeros((2, train_set_y.shape[0]))
    train_y_cato[train_set_y, np.arange(train_set_y.shape[0])] = 1.0
    test_y_cato = np.zeros((2, test_set_y.shape[0]))
    test_y_cato[test_set_y, np.arange(test_set_y.shape[0])] = 1.0
    valid_y_cato = np.zeros((2, valid_set_y.shape[0]))
    valid_y_cato[valid_set_y, np.arange(valid_set_y.shape[0])] = 1.0

    return train_set_x, train_y_cato, \
           valid_set_x, valid_y_cato, \
           test_set_x, test_y_cato
...
def build_model():
        ...
        conv1 = LeNetConvPoolLayer(
            rng=rng,
            input_shape=(batch_size, 1, 48, 96),
...
class CNN(object):
    ...
    def draw_and_save_ROC(self, X, Y):
        ...
        for k in range(batch_test_num):
            ...
            thr[k * self.batch_size:(k + 1) * self.batch_size] = A[1, :]
...
train_set_x, train_set_y, valid_set_x, valid_set_y, test_set_x, test_set_y = load_basketball_data()
